...
// Resources
const resources = () => {
  return src(path.resources.src).pipe(dest(path.resources.dist));
};
...
const fonts = () => {
  src(path.fonts.src)
    ...
    .pipe(ttf2woff())
    .pipe(dest(path.fonts.dist));
  return src(path.fonts.src).pipe(ttf2woff2()).pipe(dest(path.fonts.dist));
};
...
exports.default = series(
  clean,
  parallel(pages, scripts, fonts, resources, images, sprites),
  fontStyle,
  styles,
  watchFiles
);
